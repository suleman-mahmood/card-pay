...
def add_loyalty_points_for_deposit(
    user_id: str,
    ...
    with uow:
        user = uow.marketing_users.get(user_id)
        weightage = uow.weightages.get(TransactionType.PAYMENT_GATEWAY)
        
        user.add_loyalty_points(TransactionType.PAYMENT_GATEWAY, transaction_amount, weightage)

        uow.marketing_users.save(user)

def add_loyalty_points_for_push_transaction(
    user_id: str,
    transaction_amount: int,
    uow: AbstractUnitOfWork,
):
    with uow:
        user = uow.marketing_users.get(user_id)
        weightage = uow.weightages.get(TransactionType.P2P_PUSH)
        
        user.add_loyalty_points(TransactionType.P2P_PUSH, transaction_amount, weightage)

        uow.marketing_users.save(user)

def add_loyalty_points_for_pull_transaction(
    user_id: str,
    transaction_amount: int,
    uow: AbstractUnitOfWork,
):
    with uow:
        user = uow.marketing_users.get(user_id)
        weightage = uow.weightages.get(TransactionType.P2P_PULL)
...
#         user_id = get_user_id_from_wallet_id(recipient_wallet_id)
#         user = uow.marketing_users.get(user_id)
#         amount = user.calculate_cashback_amount(
#             deposit_amount = deposited_amount,
#             cashback_slabs = uow.cashback_slabs.get(),
#             )

#         payment_commands.execute_transaction(
#             sender_wallet_id = , #This will be a fixed cardpay wallet id
...
def set_cashback_slabs(
    ...
    with uow:
        ...
        if first_slab_start_amount != 0:
            cashback_slabs.insert(0,[0, first_slab_start_amount, "PERCENTAGE", 0]) 

        # catering for the last index
        if cashback_slabs[-1][1] <= cashback_slabs[-1][0]:
                raise ValueError("ending amount should be greater than starting amount")
        ...
        uow.cashback_slabs.save(slab_list)

...
class UserNameSchema(AbstractSchema):
    ...
    def validate(self):
        ...
        if not re.match(r"^[A-Za-z]{1}[a-z]{2,15}\s*[A-Za-z]{1}[a-z]{2,15}(\s*[A-Za-z]{1}[a-z]{2,15})?$", self.value):
...
class WeightageTypeSchema(AbstractSchema):
    ...
    def validate(self):
        ...
        if not re.match(r"^(POS|P2P_PUSH|P2P_PULL|VOUCHER|VIRTUAL_POS|PAYMENT_GATEWAY|CARD_PAY|CASH_BACK|REFERRAL|RECONCILIATION)$", self.value):
...
class WeightageValueSchema(AbstractSchema):
    ...
    def validate(self):
        ...
        if self.value<0:
            raise utils.CustomException("Negative weightage value passed, weightage value cannot be negative")
...
class AllCashbackSlabsSchema(AbstractSchema):
    ...
    def validate(self):
        ...
        if not re.match(r'''^\[\s*(\[\s*(\d+)\s*,\s*(\d+)\s*,\s*("PERCENTAGE"|"ABSOLUTE")\s*,\s*(\d+(\.\d+)?|\d+)\s*\])(\s*,\s*(\[\s*(\d+)\s*,\s*(\d+)\s*,\s*("PERCENTAGE"|"ABSOLUTE")\s*,\s*(\d+(\.\d+)?|\d+)\s*\]))*\s*\]$''', self.value):
